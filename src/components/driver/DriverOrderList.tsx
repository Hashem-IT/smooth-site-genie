...
const DriverOrderList: React.FC = () => {
  ...
  const renderOrderCard = (order: Order, isMyOrder: boolean = false) => <Card key={order.id} className="overflow-hidden">
      ...
      <CardFooter className="flex flex-wrap gap-2">
        {!isMyOrder && <Button onClick={() => handleBookOrder(order.id)} size="sm" className="flex items-center gap-1">
            <CheckCircle className="h-4 w-4" />
            Book This Order
          </Button>}
        
        {isMyOrder && order.status === "confirmed" && <Button onClick={() => handleMarkDelivered(order.id)} size="sm" className="flex items-center gap-1">
            <Truck className="h-4 w-4" />
            Mark Delivered
          </Button>}
        
        {isMyOrder && <Button variant="outline" size="sm" className="flex items-center gap-1" onClick={() => openChat(order)}>
  ...
  return <div>
      ...
      <Tabs defaultValue="available">
        ...
        <TabsContent value="available" className="space-y-4">
          {filteredAvailableOrders.length === 0 ? <div className="text-center p-8">
            ...
            </div> : filteredAvailableOrders.map(order => renderOrderCard(order))}
        </TabsContent>
        
        <TabsContent value="my-orders" className="space-y-4">
          {filteredUserOrders.length === 0 ? <div className="text-center p-8">
              <Package className="h-12 w-12 mx-auto text-gray-400" />
              <h3 className="mt-4 text-lg font-medium">No orders yet</h3>
              <p className="mt-2 text-gray-500">Book your first order to get started.</p>
            </div> : filteredUserOrders.map(order => renderOrderCard(order, true))}
        </TabsContent>
      </Tabs>
      
      {selectedOrder && <Dialog open={isChatOpen} onOpenChange={setIsChatOpen}>
          <DialogContent>
            <DialogHeader>
